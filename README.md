...
```bash
# 1. 进入仓库
cd /home/liudi/yt-transcribe-feishu
...
2. **超时**：所有命令默认超时 180 秒（`RUN_CMD_TIMEOUT`）。转写等待也可通过 `TINGWU_TRANSCRIBE_TIMEOUT` 调整，但默认不超过 180 秒。
3. **Chrome profile**：默认使用 `~/.config/google-chrome-tingwu`。如果实际 profile 路径不同，请在 `.env` 中设置 `CHROME_PROFILE_DIR`。
4. **登录态**：若登录态过期，先手动登录通义听悟，然后运行 `python -m yt_transcribe_feishu.tingwu --save-login` 保存状态。
